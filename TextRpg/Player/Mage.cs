...
namespace TextRpg.Player
{
    public class Mage : Job
    {
        ...
        public Mage(string id, string name, int level, int exp, int maxexp, int strength, int agility, int intelligence, int health, int mana, int gold, List<int> items) : base(id, name, "마법사", level, exp, maxexp, strength, agility, intelligence, health, mana, 0, 0, gold, 0, 0, items, false)
        {
            if (Health <= 0)
            {
                IsDead = true;
